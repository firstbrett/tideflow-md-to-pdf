...
pub fn rewrite_image_paths_in_markdown(
    ...
    fn absolute_norm<'a>(
        ...
        if let Some(content_root) = content_root_opt {
            ...
            if path_str.starts_with(&content_root_str) {
                ...
                if wrap_for_markdown
                    && (had_angle || rel.contains(' ') || rel.contains('(') || rel.contains(')'))
                {
                    rel = format!("<{}>", rel);
                }
    ...
    let result = re_html_img.replace_all(&result, |caps: &regex::Captures| {
        ...
        if !file_exists {
            ...
            let alt_match = Regex::new(r#"alt=([\"'])([^\"']*)\1"#)
                .ok()
            ...
            format!(
                "[⚠ Image not found: {}]",
                if alt_match.is_empty() { src } else { alt_match }
            )
        } else {
            format!(
                "<img{} src={}{}{}{}>",
                before, quote, abs, after_quote, after
            )
        }
    });
